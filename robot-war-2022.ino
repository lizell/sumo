...
#define SEARCH_TIME_THRESHOLD 1000 // milliseconds before giving up and going for a new location
...
const uint8_t chargeSensorThreshold = 4;
...
void loop()
{
  ...
  else
  {
    if (!isCharging) {
      ...
      else if (searchTime != 0 && (millis() - searchTime) > (SEARCH_TIME_THRESHOLD + random(-250, 250))) {
        charge(true);
        searchTime = 0;
      }
      else
